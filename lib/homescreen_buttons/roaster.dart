...
import 'package:train_service/main.dart';
...
class Roaster extends StatefulWidget {
  List getR;
  final String uid;
  Roaster({Key key, this.getR, this.uid}) : super(key: key);
  @override
  _RoasterState createState() => _RoasterState();
}

class _RoasterState extends State<Roaster> {
  String sidate, searchLabel = "ADD DETAILS";
  int option = 1;
  ...
  Future<Null> refreshList() async {
    Query(
        ...
        builder: (
          ...
          widget.getR = result.data["getUserRoasterslist"];
          print("Roaster _id refresh : ${widget.uid}");
        });
    await Future.delayed(Duration(seconds: 2));
    return null;
  }

  Widget roasterBody(List<dynamic> getR) {
    var screenWidth = MediaQuery.of(context).size.width;
    var screenHeight = MediaQuery.of(context).size.height;
    ...
    return GFSearchBar(
      searchBoxInputDecoration: InputDecoration(
          ...
          labelStyle: TextStyle(
              ...
              fontSize: 13,
              fontWeight: FontWeight.bold),
          hintText: "add details to Your Roaster".toUpperCase(),
          hintStyle: TextStyle(
        ...
        children: [
          FlatButton(
              ...
              onPressed: () {
                ...
                String minute = stime.substring(3);
                var scheduletime = schedulerNotificationTime(hour, minute);
                print("ALARM TIME :$scheduletime");
                _showCalender(az, uid, scheduletime);
              }),
          FlatButton(
            color: Color(0xFFF95F62),
            minWidth: 160,
  ...
  _showCalender(String az, uid, String scheduletime) {
    showDialog(
        context: context,
...
String schedulerNotificationTime(String hour, String minute) {
  if (int.parse(minute) < 30) {
    // print("SCHEDULE : ${int.parse(hour) - 1}:${int.parse(minute) + 30}");
    return "${int.parse(hour) - 1}:${int.parse(minute) + 30}";
  } else {
    // print("SCHEDULE : ${int.parse(hour)}:${int.parse(minute) - 30}");
    return "${int.parse(hour)}:${int.parse(minute) - 30}";
  }
}
