...
// class RoasterFF extends StatefulWidget {
//   final String uid;
//   const RoasterFF({ this.uid}) ;
  
//   @override
//   _RoasterFFState createState() => _RoasterFFState();
// }

// class _RoasterFFState extends State<RoasterFF> {
...
//                 if(result.loading)return RoasterF(getR: [],);

                

//                 getR=result.data["getUserRoasterFslist"];

//                 return RoasterF(getR:getR,uid:widget.uid);
...
class _RoasterFState extends State<RoasterF> {
  ...
  String sidate,searchLabel="SEARCH ROASTERF";int option=1;List<dynamic>getR;
  List getSetforRoasterF,trainListR;
      ...
      child:Scaffold(
        ...
        borderRadius: BorderRadius.only(topLeft: Radius.circular(40),topRight: Radius.circular(40),)),
          ...
          builder:(  QueryResult result, {Refetch refetch,FetchMore fetchMore,}) {
              ...
              Padding(padding: const EdgeInsets.all(15),child: head("RoasterF"),),

              setSearch(),
            
              Expanded(child: ListView.builder(itemCount: getR.length,
              itemBuilder: (context,i){
                var a=DateTime.parse("${getR[i]["duty_date"]}").toLocal();
                return  GestureDetector(
                  onTap: (){showDialog(context: context,
                      builder: (context) => AlertDialog(backgroundColor: Colors.white,contentPadding: EdgeInsets.all(5),
                        content: Container(height: 400,width:screenWidth,child: Copy(rid:getR[i]["_id"])),
                        title:Center(child: Text('    RoasterF Details    '.toUpperCase(),
                 ...
                 child:DateTime(a.year,a.month,a.day).compareTo(DateTime(DateTime.now().year,DateTime.now().month,DateTime.now().day,))==0
                    ...
                    ? Column(
                      children: [
                        roasterFlist("   ongoing   ", getR[i]["setno"], getR[i]["actual_sign_on"],
                            getR[i]["actual_sign_off"], getR[i]["user"], getR[i]["_id"], a,Colors.green[800]),
                            Container(height: 1,width: double.infinity,color: Color(0XFF8080C5).withOpacity(0.19),),
                        Align(alignment: Alignment.topLeft,
                                                  child: GestureDetector(onTap:()=>Navigator.push(context,MaterialPageRoute(builder: (_)=>
                          
                          AddDuty(did:widget.uid,drid:getR[i]["_id"]))), child: Text(" Add to Duty Records  ",style:TextStyle(backgroundColor: Colors.red[100]))
        ...
        SizedBox(height:10)
                    ...
                    : (DateTime(a.year,a.month,a.day).compareTo(DateTime.now())==1
                      ...
                      ? Column(
                        children: [
                          roasterFlist(" upcoming ", getR[i]["setno"], getR[i]["actual_sign_on"],
                              getR[i]["actual_sign_off"], getR[i]["user"], getR[i]["_id"], a,Colors.red[900]),
                              Container(height: 1,width: double.infinity,color:  Color(0XFF8080C5).withOpacity(0.19),),
        SizedBox(height:10)
                        ],
                      )
                      ...
                      :Column(
                        children: [
                          roasterFlist("completed", getR[i]["setno"], getR[i]["actual_sign_on"],
                            getR[i]["actual_sign_off"], getR[i]["user"], getR[i]["_id"], a,Color(0XFF47525E)),
                            Container(height: 1,width: double.infinity,color:  Color(0XFF8080C5).withOpacity(0.19),),
        SizedBox(height:10)
                        ],
                      ))
...
Widget roasterFlist(String label,setname,signon,signoff,userid,roasterFid,DateTime datename,Color zz) {
  ...
  return Column(crossAxisAlignment: CrossAxisAlignment.end,children: [
      ...
      Container(height: 60,color:Color(0XFFFDFFFC).withOpacity(0.6),padding: EdgeInsets.only(left:10,right:10),
                  ...
                  GestureDetector(child: Icon(Icons.edit,size:15),onTap: (){var snh,snm,sfh,sfm;
                      showTimePicker(context: context, initialTime: TimeOfDay.now(),confirmText: "Add Sign On").then((value1) {
                        ...
                        showTimePicker(context: context, initialTime: TimeOfDay.now(),confirmText: "Add Sign Off").then((value2){
                          ...
                          MaterialPageRoute(builder: (_)=>Mut1(snh:snh,snm:snm,sfh:sfh,uID:userid,sfm:sfm,id:roasterFid)));});});},),
SizedBox(height:5)
    ...
    return GraphQLProvider(      
      ...
      child:Query(
        ...
        builder:(  QueryResult result, {Refetch refetch,FetchMore fetchMore,}) {
          ...
          getSetforRoasterF=result.data["getAllSets"];
            return roasterFSearch(getSetforRoasterF);
                        
                    
        }
      ),);
}
Widget roasterFSearch(List xz){
   var screenWidth=MediaQuery.of(context).size.width;
  return  GFSearchBar(
        ...
        hintText: "Enter Set No To search Your RoasterF",hintStyle: TextStyle(color: Color(0XFF011627),fontFamily: "Roboto",fontSize: 11),
    ...
    child: Row(
      ...
      children: [
        FlatButton(color: Color(0xFF8FB339),minWidth: 160,child: Row(mainAxisAlignment: MainAxisAlignment.start,children: [
                ...
                Text('Add To RoasterF', style: const TextStyle(color: Color(0XFFFDFFFC),fontSize: 11)),],),
                onPressed:() {
    ...
    return GraphQLProvider(
      ...
      body: Mutation(
        options:MutationOptions(
          document: r"""
            mutation UpdateRoasterF($_id:ID!,$actual_sign_on:String!,$actual_sign_off:String!){
              UpdateRoasterF(roasterFInput:{_id:$_id,actual_sign_on:$actual_sign_on,actual_sign_off:$actual_sign_off}){
    ...
    return GraphQLProvider(
      ...
      child: Query(
        options:QueryOptions(
          document: r"""
            query getid($_id:ID!){
              getUserRoasterFDetails(_id:$_id){
                _id
                duty_date
                setno                
                sign_on
        ...
        builder:(  QueryResult result, {Refetch refetch,FetchMore fetchMore,}) {
          ...
          zz=result.data["getUserRoasterFDetails"]["RoasterFdetails"];
          ...
          child: Padding(padding:const EdgeInsets.fromLTRB(5, 10, 5,0),
            ...
            children: [
              ...
              child: Padding(padding: const EdgeInsets.only(top: 5),
                child: Row(
                  children: [
                  Padding(padding: const EdgeInsets.only(left:5.0),child: frowlist("sign on",result.data["getUserRoasterFDetails"]["actual_sign_on"],
                    result.data["getUserRoasterFDetails"]["start_station_code"]),),  

                  SizedBox(width:5),Dash(direction:Axis.vertical,dashColor: Color(0XFF505FE1),length: 50,),SizedBox(width:4),
                  frowlist("sign off",result.data["getUserRoasterFDetails"]["actual_sign_off"],
                  result.data["getUserRoasterFDetails"]["end_station_code"]),

                  SizedBox(width:5),Dash(direction:Axis.vertical,dashColor: Color(0XFF505FE1),length: 50,),SizedBox(width:4),
                  frowlist("duty hrs",result.data["getUserRoasterFDetails"]["actual_duty_hrs"],""),

                  SizedBox(width:5),Dash(direction:Axis.vertical,dashColor: Color(0XFF505FE1),length: 50,),SizedBox(width:4),
                  frowlist("kms",result.data["getUserRoasterFDetails"]["distance"],""),

                  SizedBox(width:5),Dash(direction:Axis.vertical,dashColor: Color(0XFF505FE1),length: 50,),SizedBox(width:4),
                  frowlist("ndh",result.data["getUserRoasterFDetails"]["ndh"],""),],),)),

              SizedBox(height: 5,),
              Container(width:70,height: 45,
              decoration: BoxDecoration(color: Color(0XFFFDFFFC).withOpacity(0.6),borderRadius: BorderRadius.circular(10),),
              child: Center(child:Text(result.data["getUserRoasterFDetails"]["setno"],
